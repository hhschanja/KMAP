...
<form id="frm" method="get" action="./purchaseBuy.purchase">
<table>
<tr>
...
<c:forEach items="${basket}" var="v" varStatus="i">

<input type="hidden" value="${v.count * v.price}" id="totalP${i.index}">
<input type="hidden" value="${v.num}" name="ckck${i.index}" >

<tr>
<td class="table_short_first"><input type="checkbox" value="${v.num}" name="ck${i.index}" class="totalPrice" title="${i.index}"></td>
<td class="table_middle"><img style="width: 100px; height: 120px;" alt="" src="../upload/${files[i.index]}"> </td>
<td class="table_long">${v.name} </td>
<td class="table_short_second">${v.count} </td>
<td class="table_short">
<fmt:formatNumber value="${v.price}" type="currency" currencySymbol=""/> 원
</td>
<td class="table_middle">
<fmt:formatNumber value="${v.count * v.price}" type="currency" currencySymbol=""/> 원
 </td></tr>

<input type="hidden" name="t${i.index }" value="${v.type_num}">
<input type="hidden" name="a${i.index }" value="${i.count }"><!--총 수량 구하기 위한 히든 건드리지 말것!  -->
